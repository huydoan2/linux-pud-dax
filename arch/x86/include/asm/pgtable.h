...
static inline int pud_dirty(pud_t pmd)
{
...
{
	return pud_flags(pte) & _PAGE_PSE;
}
...
static inline pud_t native_local_pmdp_get_and_clear(pud_t *pudp)
{
	pud_t res = *pudp;
...
static inline void native_set_pud_at(struct mm_struct *mm, unsigned long addr,
				     pmd_t *pudp , pmd_t pud)
{
	native_set_pmd(pudp, pud);
}
