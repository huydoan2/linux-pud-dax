...
#define RADIX_DAX_PUD (1 << (RADIX_TREE_EXCEPTIONAL_SHIFT + 3))
#define RADIX_DAX_TYPE_MASK (RADIX_DAX_PTE | RADIX_DAX_PMD | RADIX_DAX_PUD)
#define RADIX_DAX_TYPE(entry) ((unsigned long)entry & RADIX_DAX_TYPE_MASK)
#define RADIX_DAX_SECTOR(entry) (((unsigned long)entry >> RADIX_DAX_SHIFT))
#define RADIX_DAX_ENTRY(sector, pmd) ((void *)((unsigned long)sector << \
		RADIX_DAX_SHIFT | \
		((pmd & 0x1) ? RADIX_DAX_PTE : 0x0) | \
		(((pmd >> 1) & 0x1) ? RADIX_DAX_PMD : 0x0) | \
		(((pmd >> 2) & 0x1) ? RADIX_DAX_PUD : 0x0) | \
...
#define DAX_PUD_INDEX(page_index) (page_index & (PUD_MASK >> PAGE_SHIFT))
				      ...
				      void *entry, sector_t sector)
{
	...
	*/
	///MODIFY
	new_entry = (void *)((unsigned long)RADIX_DAX_ENTRY(sector, 0x1) |
		       RADIX_DAX_ENTRY_LOCK);
		...
		struct address_space *mapping, pgoff_t index, void *entry)
{
	...
	if (WARN_ON_ONCE(type != RADIX_DAX_PTE && type != RADIX_DAX_PMD && type != RADIX_DAX_PUD)) {
		ret = -EIO;
		goto unlock;
	}
	...
	/// MODIFY
	if(type == RADIX_DAX_PUD)
		dax.size = PUD_SIZE;
	else if( type == RADIX_DAX_PMD)
		dax.size = PMD_SIZE;
	else
		dax.size = PAGE_SIZE;
	///
		...
		struct block_device *bdev, struct writeback_control *wbc)
{
	...
	pgoff_t start_index, end_index, pmd_index, pud_index;
	...
	rcu_read_lock();
	entry = radix_tree_lookup(&mapping->page_tree, pud_index);
	rcu_read_unlock();

	/* see if the start of our range is covered by a PUD entry */
		...
		pud_t *pud, unsigned int flags, get_block_t get_block)
{
	...
	if (!IS_ENABLED(CONFIG_FS_DAX_PUD))
		return VM_FAULT_FALLBACK;
	...
	if (!write && !buffer_mapped(&bh)) {
		...
		if (!pud_none(*pud)) {
			spin_unlock(ptl);
			dax_pud_dbg(&bh, address, "pud already present");
			goto fallback;
		}
		...
		set_pud_at(vma->vm_mm, pud_addr, pud, entry);
		result = VM_FAULT_NOPAGE;
		spin_unlock(ptl);
	} else {
		struct blk_dax_ctl dax = {
		...
		if (length < 0) {
			dax_pud_dbg(&bh, address, "dax-error fallback");
			goto fallback;
		}
		if (length < PUD_SIZE) {
		 ...
		 * order to be able to create a new radix tree entry.
		 *
		 * The PUD path doesn't have an equivalent to
		 * dax_pfn_mkwrite(), though, so for a read followed by a
		 * write we traverse all the way through dax_pud_fault()
		 * twice.  This means we can just skip inserting a radix tree
		 * entry completely on the initial read and just wait until
		 * the write to insert a dirty entry.
		 */
